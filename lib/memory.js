...
function all(sql, ...params) {
  if (adapter === 'node') {
    ...
    return db.all(sql, params.length > 0 ? params : undefined)
  }
}
...
export async function initDB(dbFile = './data/oc-plugin.db') {
  ...
  try {
    ...
    try {
      ...
      // Wait for WASM runtime initialization if not already done
      await new Promise(resolve => {
        if (sqlite3.calledRun) {
          resolve()
        } else {
          const orig = sqlite3.onRuntimeInitialized
          sqlite3.onRuntimeInitialized = () => {
            if (orig) orig()
            resolve()
          }
          // Safeguard in case we missed it
          setTimeout(resolve, 50)
        }
      })

      db = new sqlite3.Database(absPath)
      adapter = 'wasm'
...
export function claimTask(agentId, role = null) {
  ...
  // Atomic claim: update the first matching pending row and return the number of changes
  let changes = 0
  if (adapter === 'node') {
    const result = db.prepare(`
      UPDATE tasks SET agent_id = ?, status = 'running', started_at = datetime('now')
      WHERE id = (
    ...
    // try/catch block ensures we can adapt to node-sqlite3-wasm correctly
    // It returns the statement info object but the exact API for rows changed varies
    // However, an even safer approach that definitely works across adapters is
    // to just perform the query then check if our agent _actually_ won.

    // Select a candidate
    const qParams = role ? [role] : []
    const candidate = get(
      `SELECT id FROM tasks WHERE status = 'pending' ${roleFilter} ORDER BY priority DESC, created_at ASC LIMIT 1`,
      ...qParams
    )
    if (!candidate) return null

    // Try to claim the exact candidate
    run(
      `UPDATE tasks SET agent_id = ?, status = 'running', started_at = datetime('now')
       WHERE id = ? AND status = 'pending'`,
      agentId, candidate.id
    )
    changes = 1 // Optimistic, verification step below guarantees correctness
  }

  // Verify we actually got it (solves node-sqlite3-wasm race condition or unconfirmed updates)
  const claimed = get(
